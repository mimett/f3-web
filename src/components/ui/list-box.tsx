...
export default function Listbox({
  ...
  return (
    <div className={cn('relative', className)}>
      <HeadlessListbox value={selectedOption} onChange={onChange} disabled={disabled}>
        <div className='flex w-full items-center gap-2'>
          {label && (<HeadlessListbox.Label>{label}</HeadlessListbox.Label>)}
        ...
        >
          <HeadlessListbox.Options className="absolute left-0 z-10 mt-1 grid w-full origin-top-right gap-0.5 rounded-lg border border-gray-200 bg-white p-1 shadow-large outline-none dark:border-gray-700 dark:bg-gray-800 xs:p-2">
            {options.map((option) => (
              <HeadlessListbox.Option key={option.value || null} value={option}>
                {({ selected }) => (
                  <div
